...
export default function CategoryManagement() {
  ...
  const handleSubmit: React.SubmitEventHandler<HTMLFormElement> = async (e) => {
    e.preventDefault();
    try {
      await categorySchema.validate(formValues, { abortEarly: false });
      setFormErrors({});
    } catch (err) {
      if (err instanceof yup.ValidationError) {
        const fieldErrors: FormErrors = {};
        err.inner.forEach((e) => {
          if (e.path && !fieldErrors[e.path as keyof CategoryFormValues]) {
            fieldErrors[e.path as keyof CategoryFormValues] = e.message;
          }
        });
    ...
    try {
      if (modalMode === "create") {
        ...
        AppAlert({
          icon: "success",
          title: res?.message || "Đã thêm danh mục",
        });
        ...
        AppAlert({
          icon: "success",
          title: res?.message || "Đã cập nhật danh mục",
        });
      }
      setModalOpen(false);
      fetchCategories();
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await categoryApi.removeCategory(deleteTarget.id);
      AppAlert({ icon: "success", title: res?.message || "Đã xóa danh mục" });
      ...
      AppAlert({
        icon: "error",
        title: (await getApiErrorMessage(err)) || "Không thể xóa danh mục",
      });
    } finally {
      setDeleting(false);
    }
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-[#30363d]">
            {loading ? (
              ...
              categories.map((c) => (
                ...
                >
                  <td className="px-4 py-3.5 font-medium text-slate-700 dark:text-[#e6e9ef]">
                    {c.name}
                  </td>
                  <td className="px-4 py-3.5">
                    <code className="text-[12px] text-slate-500 dark:text-[#9aa4b2] bg-slate-50 dark:bg-[#161b26] px-1.5 py-0.5 rounded">
                      {c.slug}
                    </code>
                  </td>
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} noValidate>
              <FormField label="Tên danh mục" error={formErrors.name}>
                <input
              ...
              <FormField
                ...
                hint="Để trống để hệ thống tự sinh từ tên danh mục."
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-[#8b95a8] mt-1.5">
                  ...
                  {deleteTarget._count?.products
                    ? " Danh mục đang có sản phẩm sẽ không thể xóa cho tới khi chuyển hết sản phẩm sang danh mục khác."
